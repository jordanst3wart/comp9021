...
class Solitaire(Tk):
    def __init__(self):
        ...
        self.select_or_create_problem_button = Button(buttons, text = 'Select defined problem',
                                                      width = 16,
                                                      command = self.select_or_create_problem)
        self.select_or_create_problem_button.pack(padx = 30, side = LEFT)
        self.prepare_to_play_or_play_button = Button(buttons, text = 'Start playing',
                                                     command = self.prepare_to_play_or_play)
        self.prepare_to_play_or_play_button.pack(padx = 30, side = LEFT)
        self.clear_or_demonstrate_button = Button(buttons, width = 16, text = 'Clear',
...
class Board(Frame):
    ...
    def draw_board(self):
        self.board.create_polygon(3 * GAP, 2 * GAP - 5, 5 * GAP + 10, 2 * GAP - 5, 7 * GAP + 15,
                                  4 * GAP, 7 * GAP + 15, 6 * GAP + 10, 5 * GAP + 10, 8 * GAP + 15,
                                  3 * GAP, 8 * GAP + 15, GAP - 5, 6 * GAP + 10, GAP - 5, 4 * GAP,
                                   fill = BOARD_COLOUR, outline = 'black', width = 2)
    ...
    def represent_configuration(self):
        for p in self.configuration.positions:
            if self.configuration.positions[p]:
                self.board.itemconfig(self.drawn_configuration[p], fill = PEG_COLOUR,
                                      outline = PEG_COLOUR)
    ...
    def select_peg_or_jump_and_take(self, event):
        ...
        if self.selected_peg:
            ...
            self.board.itemconfig(self.drawn_configuration[p], fill = SELECTION_COLOUR,
    ...
    def demonstrate(self):
        if self.selected_peg:
            ...
            self.board.itemconfig(self.drawn_configuration[self.to_position], fill = PEG_COLOUR,
                                  outline = PEG_COLOUR)
            ...
            if not self.selected_solution:
                ...
                self.board.itemconfig(self.drawn_configuration[self.from_position],
                                      fill = SELECTION_COLOUR, outline = SELECTION_COLOUR)
